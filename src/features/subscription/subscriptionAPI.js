    ...
    getAllSubscriptions: builder.query({
      ...
      async onQueryStarted(query, { queryFulfilled, dispatch }) {
        try {
          ...
          dispatch(
            addMessage({
              message: error?.error?.data?.message || error.message || 'Failed to load subscription',
              type: 'error',
            })
          );
        }
    ...
    createSubscription: builder.mutation({
      ...
      async onQueryStarted(query, { queryFulfilled, dispatch }) {
        try {
          ...
          const { data } = await queryFulfilled;
          dispatch(addSubscription(data?.data));
    ...
    cancelSubscription: builder.mutation({
      ...
      async onQueryStarted(query, { queryFulfilled, dispatch }) {
        try {
          ...
          const { data } = await queryFulfilled;
          dispatch(cancelSubscription({prevId: query?._id, newData: data?.data}));

          dispatch(
            addMessage({
              message: 'Subscription cacelled successfully',
              type: 'success',
            })
          );
          ...
          dispatch(
            addMessage({
              message: error?.error?.data?.message || error.message || 'Subscription cancellation failed',
              type: 'error',
            })
          );
        }
